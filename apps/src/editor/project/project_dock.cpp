...
ProjectDock::ProjectDock(QWidget *parent)
    : QDockWidget(parent),
      ...
      m_view(new qtils::QtUnselectableTreeView(this)),
      m_model(new QFileSystemModel(this)),
      m_proxy(new qtils::FileSystemProxyModel(this)),
...
void ProjectDock::openFile(const QModelIndex &index) {
  ...
  QStringList args = QStringList() << QDir::toNativeSeparators(path);
  QProcess::startDetached(explorer, args);
#elif defined(Q_OS_LINUX)
  QString fileManager = "xdg-open";
  QStringList args = QStringList() << QDir::toNativeSeparators(path);
...
void ProjectDock::removeFile(const QModelIndex &index) {
  ...
  auto remove_method =
      is_file
          ? std::function<bool()>([&path]() { return QFile(path).remove(); })
...
void ProjectDock::newDirectory(const QModelIndex &index) {
  ...
  const auto new_name = QInputDialog::getText(this, tr("Create Directory"),
                                              tr("Directory Name:"));
