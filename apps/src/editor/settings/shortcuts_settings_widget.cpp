...
#include "settings/shortcuts_table_delegate.h"
...
ShortcutsSettingsWidget::ShortcutsSettingsWidget(QWidget *parent)
    : SettingsWidget(parent),
      m_ui(new Ui::ShortcutsSettingsWidget()),
...
void ShortcutsSettingsWidget::initConnections() {
  connect(m_ui->m_shortcuts_search, &QLineEdit::textChanged, this,
...
void ShortcutsSettingsWidget::contextMenu(const QPoint &pos) {
  ...
  if (!selected_rows.empty()) {
    ...
    menu->addAction(tr("Clear"),
                    [selected_rows, this]() { clearShortcuts(selected_rows); });
...
void ShortcutsSettingsWidget::setDefaultShortcuts(
  ...
  for (const auto &index : indexes) {
    ...
    const auto action_default_shortcut =
        pillar::ActionManager::getInstance().getDefaultShortcut(action_id);
...
void ShortcutsSettingsWidget::clearShortcuts(
    const QList<QModelIndex> &indexes) {
  for (const auto &index : indexes) {
    const auto action_shortcut_index = m_search_proxy_model->index(
        index.row(), ShortcutsTableModel::Column::ShortcutColumn);

    m_search_proxy_model->setData(action_shortcut_index, QVariant{},
                                  Qt::DisplayRole);
  }
}
...
std::unique_ptr<pillar::SettingsWidget> ShortcutsSettingsWidgetFactory::create()
    const {
