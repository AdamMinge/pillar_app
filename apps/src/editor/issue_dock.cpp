...
#include "issue_table_filter_model.h"
...
void IssueDock::initConnections() {
  connect(m_ui->m_issues_view, &QTableView::activated, this,
          &IssueDock::issueActivate);
  connect(m_ui->m_search_issue, &QLineEdit::textChanged, this,
          &IssueDock::searchIssue);

  connect(m_ui->m_show_warnings_button, &QToolButton::clicked,
          m_issue_filter_model.get(),
          &IssueTableFilterModel::setWarningsVisible);
  connect(m_ui->m_show_errors_button, &QToolButton::clicked,
