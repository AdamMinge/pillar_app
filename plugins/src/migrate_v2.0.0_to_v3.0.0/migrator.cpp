...
QVersionNumber Migrator::fromVersion() const { return QVersionNumber(2, 0, 0); }

QVersionNumber Migrator::toVersion() const { return QVersionNumber(3, 0, 0); }

QByteArray Migrator::migrate(const QByteArray &from) const
{
  auto snapshot_from = snap_v2_0_0::Snapshot{};
  auto snapshot_to = snap_v3_0_0::Snapshot{};
  ...
  snapshot_to.set_id(snapshot_from.name());
  snapshot_to.set_nr(snapshot_from.nr());
...
{
  auto snapshot_from = snap_v3_0_0::Snapshot{};
  ...
  snapshot_to.set_name(snapshot_from.id());
  snapshot_to.set_nr(snapshot_from.nr());
