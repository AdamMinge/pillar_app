...
void GroupLayer::serialize(qtils::OArchive& archive) const {
  Layer::serialize(archive);

  archive << qtils::ArchiveProperty("layers", m_layers);
}

void GroupLayer::deserialize(qtils::IArchive& archive) {
  Layer::deserialize(archive);

  auto layers = Layers{};
  archive >> qtils::ArchiveProperty("layers", layers);
