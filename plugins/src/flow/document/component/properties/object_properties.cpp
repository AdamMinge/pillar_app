...
/* ----------------------------------- Qtils -------------------------------- */
#include <qtils/property_browser/editor_factory.h>
#include <qtils/property_browser/property_manager.h>
...
Properties::Properties(const QString& name, QObject* parent)
    : QObject(parent),
      m_group_manager(new qtils::QtGroupPropertyManager(this)),
      m_property_manager(new VariantPropertyManager(this)),
      m_editor_factory(new qtils::QtVariantEditorFactory(this)),
...
qtils::QtProperty* Properties::getRoot() const { return m_root; }

qsizetype Properties::size() const { return m_property_to_id.size(); }

bool Properties::contains(qtils::QtProperty* property) const {
  if (!property) return false;
  return m_property_to_id.contains(property);
}

void Properties::setFactoryForManager(qtils::QtTreePropertyBrowser* browser) {
  browser->setFactoryForManager(m_property_manager, m_editor_factory);
}

void Properties::unsetFactoryForManager(qtils::QtTreePropertyBrowser* browser) {
  browser->unsetFactoryForManager(m_property_manager);
}

qtils::QtProperty* Properties::createGroup(const QString& name,
                                           qtils::QtProperty* parent) {
...
qtils::QtVariantProperty* Properties::createProperty(
    size_t id, int type, const QString& name, qtils::QtProperty* parent) {
...
qtils::QtVariantProperty* Properties::getPropertyById(size_t id) const {
...
QList<qtils::QtProperty*> ObjectProperties::getProperties() const {
...
bool ObjectProperties::isCustomProperty(qtils::QtProperty* property) const {
  return m_custom_properties->contains(property);
}

void ObjectProperties::setFactoryForManager(
    qtils::QtTreePropertyBrowser* browser) {
