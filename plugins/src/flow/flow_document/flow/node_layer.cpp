...
void NodeLayer::serialize(qtils::OArchive& archive) const {
  Layer::serialize(archive);

  archive << qtils::ArchiveProperty("nodes", m_nodes);
  archive << qtils::ArchiveProperty("connections", m_connections);
}

void NodeLayer::deserialize(qtils::IArchive& archive) {
  Layer::deserialize(archive);

  auto nodes = Nodes{};
  archive >> qtils::ArchiveProperty("nodes", nodes);
  for (auto&& node : nodes) appendNode(std::move(node));

  auto connections = Connections{};
  archive >> qtils::ArchiveProperty("connections", connections);
