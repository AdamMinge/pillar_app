...
namespace flow
{
  ...
  class LIB_FLOW_API PreferencesSettings
  {
  public:
    enum class Type
    {
  ...
  class LIB_FLOW_API PreferencesManager : public QObject
  {
    Q_OBJECT
  ...
  template<typename TYPE>
  class LIB_FLOW_API PreferenceContainer
  {
  public:
  ...
  template<typename TYPE>
  PreferenceContainer<TYPE>::PreferenceContainer(
    QString key, TYPE default_value)
      : m_key(std::move(key)), m_default_value(default_value)
  {}
