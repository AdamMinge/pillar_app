...
namespace flow
{
  ...
  inline void REPORT_WARNING(
    QString text, QVariant context = QVariant{},
    Issue::Callback callback = Issue::Callback{})
  {
    IssueManager::getInstance().report(Issue{
      Issue::Severity::Warning, std::move(text), std::move(context),
      std::move(callback)});
  }
  ...
  {
    IssueManager::getInstance().report(Issue{
      Issue::Severity::Error, std::move(text), std::move(context),
      std::move(callback)});
  }
