...
public:
  ...
  [[nodiscard]] FORMAT *findFormatByNameFilter(const QString &name_filter) const;
...
template<typename FORMAT>
FORMAT *FormatHelper<FORMAT>::findFormatByFileName(const QString &file_name) const
{
  auto found_format = std::find_if(m_formats.begin(), m_formats.end(), [&file_name](auto format) {
...
template<typename FORMAT>
FORMAT *FormatHelper<FORMAT>::findFormatByShortName(const QString &short_name) const
{
  auto found_format = std::find_if(m_formats.begin(), m_formats.end(), [&short_name](auto format) {
...
template<typename FORMAT>
FORMAT *FormatHelper<FORMAT>::findFormatByNameFilter(const QString &name_filter) const
{
  auto found_format = std::find_if(m_formats.begin(), m_formats.end(), [&name_filter](auto format) {
