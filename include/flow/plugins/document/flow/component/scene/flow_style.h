...
private:
  ...
  const TYPE &
  getProperty(std::optional<TYPE> Data::*property, States states) const;
...
template<typename TYPE>
const TYPE &
PinStyle::getProperty(std::optional<TYPE> Data::*property, States states) const
{
  ...
  Q_ASSERT_X(
    (m_state_data.at(State::Normal).*property).has_value(),
...
public:
  ...
  [[nodiscard]] const std::array<float, 4> &
  getGradientScale(States states) const;
  [[nodiscard]] const QColor &getBorderColor(States states) const;
...
template<typename TYPE>
const TYPE &
NodeStyle::getProperty(std::optional<TYPE> Data::*property, States states) const
{
  for (auto state : {State::Hovered, State::Selected})
  {
    if (!states.testFlag(state)) continue;
...
public:
  ...
  [[nodiscard]] const NodeStyleViewer &
  getNodeStyleViewer(NodeStyle::States states) const;

  void setPinStyle(const PinStyle &style);
  [[nodiscard]] const PinStyle &getPinStyle() const;
  [[nodiscard]] const PinStyleViewer &
  getPinStyleViewer(PinStyle::States states) const;
