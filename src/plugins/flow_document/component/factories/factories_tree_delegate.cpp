...
FactoriesTreeDelegate::FactoriesTreeDelegate(QObject *parent)
    : QStyledItemDelegate(parent), m_icon_size(16, 16), m_spacing(10) {}

FactoriesTreeDelegate::~FactoriesTreeDelegate() = default;

void FactoriesTreeDelegate::paint(QPainter *painter,
  ...
  const auto node_name =
      index.data(FactoriesTreeModel::Role::NameRole).toString();
  const auto node_icon =
      index.data(FactoriesTreeModel::Role::IconRole).value<QIcon>();
...
QSize FactoriesTreeDelegate::sizeHint(const QStyleOptionViewItem &option,
                                      const QModelIndex &index) const {
  QStyleOptionViewItem opt(option);
  initStyleOption(&opt, index);
