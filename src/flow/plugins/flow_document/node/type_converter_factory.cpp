...
void TypeConverterFactories::registerFactory(
  ...
  m_factories.insert(std::make_pair(std::move(factory->getTypeConverterId()),
                                    std::move(factory)));
}

void TypeConverterFactories::unregisterFactory(
    const QString &type_converter_id) {
...
TypeConverterFactory *TypeConverterFactories::getFactory(
...
std::unique_ptr<TypeConverter> TypeConverterFactories::create(
    const QString &type_converter_id) const {
  if (!m_factories.contains(type_converter_id)) return nullptr;
