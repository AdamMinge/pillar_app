/* ----------------------------------- Local -------------------------------- */
#include "flow/plugins/flow_document/component/type_converter/flow_type_converters_tree_model.h"

#include "flow/plugins/flow_document/node/type_converter_factory.h"
/* ------------------------------------ Qt ---------------------------------- */
#include <QMimeData>
/* -------------------------------------------------------------------------- */
...
void FlowTypeConvertersTreeItem::removeChild(
    FlowTypeConvertersTreeItem *child) {
  if (m_children.contains(child)) {
...
const FlowTypeConvertersTreeItem *FlowTypeConvertersTreeItem::getChild(
    int row) const {
...
Qt::ItemFlags FlowTypeConvertersTreeModel::flags(
...
QVariant FlowTypeConvertersTreeModel::data(const QModelIndex &index,
                                           int role) const {
...
QVariant FlowTypeConvertersTreeModel::headerData(int section,
...
QModelIndex FlowTypeConvertersTreeModel::parent(
    const QModelIndex &index) const {
...
QMimeData *FlowTypeConvertersTreeModel::mimeData(
    const QModelIndexList &indexes) const {
  auto mime_data = new QMimeData;
  mime_data->setData(QLatin1String("flow/type_converter"),
                     createMimeData(indexes));
...
QByteArray FlowTypeConvertersTreeModel::createMimeData(
  ...
  for (auto index : indexes) {
    auto type_converter_factory =
        static_cast<FlowTypeConvertersTreeFactoryItem *>(
