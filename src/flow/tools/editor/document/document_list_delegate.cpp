...
DocumentListDelegate::~DocumentListDelegate() = default;
...
void DocumentListDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
  ...
  auto name = index.data(DocumentListModel::Role::DocumentNameRole).toString();
  auto path = index.data(DocumentListModel::Role::DocumentPathRole).toString();
  auto modified_date = index.data(DocumentListModel::Role::DocumentLastModifiedDateRole).toDateTime().toString();
  ...
  painter->drawPixmap(content_rect.left(), content_rect.top(),
                      index.data(DocumentListModel::Role::DocumentIconRole).value<QIcon>().pixmap(m_icon_size));
...
void DocumentListDelegate::setIconSize(const QSize& size)
{
  m_icon_size = size;
}

const QSize& DocumentListDelegate::getIconSize() const
{
  return m_icon_size;
}

void DocumentListDelegate::setMargins(const QMargins& margins)
{
  m_margins = margins;
}

const QMargins& DocumentListDelegate::getMargins() const
{
  return m_margins;
}

void DocumentListDelegate::setSpacing(int vertical_spacing, int horizontal_spacing)
{
  m_spacing.setWidth(vertical_spacing);
  m_spacing.setHeight(horizontal_spacing);
}

void DocumentListDelegate::setVerticalSpacing(int spacing)
{
  m_spacing.setWidth(spacing);
}

void DocumentListDelegate::setHorizontalSpacing(int spacing)
{
  m_spacing.setHeight(spacing);
}

int DocumentListDelegate::getVerticalSpacing() const
{
  return m_spacing.width();
}

int DocumentListDelegate::getHorizontalSpacing() const
{
