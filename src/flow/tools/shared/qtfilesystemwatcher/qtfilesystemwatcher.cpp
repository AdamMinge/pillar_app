...
#include <QFile>
/* -------------------------------- Tools Shared ---------------------------- */
#include "flow/tools/shared/qtfilesystemwatcher/qtfilesystemwatcher.h"
/* -------------------------------------------------------------------------- */

namespace tools {

QtFileSystemWatcher::QtFileSystemWatcher(QObject* parent) :
  QObject(parent)
{
  ...
  connect(std::addressof(m_watcher), &QFileSystemWatcher::fileChanged,
          this, &QtFileSystemWatcher::onFileChanged);
  ...
  connect(std::addressof(m_changed_paths_timer), &QTimer::timeout,
          this, &QtFileSystemWatcher::pathsChangedTimeout);
}
...
void QtFileSystemWatcher::clear()
{
