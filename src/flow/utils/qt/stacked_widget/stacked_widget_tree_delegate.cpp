...
namespace utils
{
  ...
  void StackedWidgetTreeDelegate::paint(
    ...
    const QModelIndex &index) const
  {
    ...
    auto content_rect = rect.adjusted(
      m_margins.left(), m_margins.top(), -m_margins.right(),
      -m_margins.bottom());
    ...
    auto color = opt.state & QStyle::State_Selected
                   ? palette.highlight().color()
                   : palette.light().color();

    painter->fillRect(
      0, rect.top(), rect.width() + rect.left(), rect.height(), color);
    ...
    style->drawPrimitive(
      QStyle::PE_IndicatorBranch, &primitive_opt, painter, nullptr);
  ...
  QSize StackedWidgetTreeDelegate::sizeHint(
    const QStyleOptionViewItem &option, const QModelIndex &index) const
  {
    ...
    return QSize{
      opt.rect.width(),
      QFontMetrics(opt.font).height() + m_margins.top() + m_margins.bottom()};
  }
