...
/* ----------------------------------- Local -------------------------------- */
#include "flow/utils/qt/property_browser/group_box_property_browser.h"
/* -------------------------------------------------------------------------- */
...
namespace utils
{
  ...
  void QtGroupBoxPropertyBrowserPrivate::slotUpdate()
  {
    for (WidgetItem *item : qAsConst(m_recreateQueue))
    {
      ...
      int span = 1;
      if (item->widget) l->addWidget(item->widget, oldRow, 1, 1, 1);
      else if (item->widgetLabel)
                                                     ...
                                                     QtBrowserItem *afterIndex)
  {
    ...
    if (parentItem && hasHeader(parentItem)) row += 2;
  ...
  void QtGroupBoxPropertyBrowserPrivate::propertyRemoved(QtBrowserItem *index)
  {
    ...
    if (item->groupBox) delete item->groupBox;
                                                   ...
                                                   int row) const
  {
    ...
    const QMap<QLayoutItem *, QRect>::ConstIterator icend =
      itemToPos.constEnd();
...
#include "flow/utils/qt/property_browser/moc_group_box_property_browser.cpp"
