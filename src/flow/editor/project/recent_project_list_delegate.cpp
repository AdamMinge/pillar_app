...
void RecentProjectListDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
  ...
  auto name = index.data(RecentProjectListModel::Role::ProjectNameRole).toString();
  auto path = index.data(RecentProjectListModel::Role::ProjectPathRole).toString();
  auto modified_date = index.data(RecentProjectListModel::Role::ProjectLastModifiedDateRole).toDateTime().toString();
  ...
  painter->drawPixmap(content_rect.left(), content_rect.top(),
                      index.data(RecentProjectListModel::Role::ProjectIconRole).value<QIcon>().pixmap(m_icon_size));
...
QSize RecentProjectListDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
  QStyleOptionViewItem opt(option);
  initStyleOption(&opt, index);
...
void RecentProjectListDelegate::setSpacing(int vertical_spacing, int horizontal_spacing)
{
