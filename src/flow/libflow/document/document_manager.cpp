...
namespace flow::document
{
  ...
  void
  DocumentManager::insertDocument(int index, std::unique_ptr<Document> document)
  {
    Q_ASSERT(document);
    ...
    connect(
      document_ptr, &Document::fileNameChanged, this,
      &DocumentManager::fileNameChanged);
    connect(
      document_ptr, &Document::modifiedChanged, this,
  ...
  bool DocumentManager::reloadDocumentAt(int index)
  {
    ...
    QString error;
    auto new_document =
      Document::load(document->getFileName(), nullptr, &error);
    if (!new_document)
    {
  ...
  bool DocumentManager::saveDocumentAs(Document *document)
  {
    ...
    auto format_helper =
      FormatHelper<document::DocumentFormat>{FileFormat::Capability::Write};
  ...
  const std::vector<std::unique_ptr<Document>> &
  DocumentManager::getDocuments() const
  {
    return m_documents;
  }

  void DocumentManager::addedObject(DocumentEditor *object)
  {
    addEditor(object);
  }
  ...
  void DocumentManager::currentIndexChanged()
  {
    ...
    if (editor)
    {
      ...
      connect(
        editor, &DocumentEditor::enabledStandardActionsChanged, this,
