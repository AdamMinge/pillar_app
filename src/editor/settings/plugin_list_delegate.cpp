...
QWidget *PluginListDelegate::createEditor(QWidget *parent,
                                          const QStyleOptionViewItem &option,
                                          const QModelIndex &index) const {
  auto editor = new QCheckBox(parent);

  connect(editor, &QCheckBox::stateChanged, this,
          [this, editor, index](auto value) {
            setModelData(
...
void PluginListDelegate::setEditorData(QWidget *editor,
                                       const QModelIndex &index) const {
...
void PluginListDelegate::setModelData(QWidget *editor,
...
void PluginListDelegate::updateEditorGeometry(
  ...
  const auto content_rect =
      rect.adjusted(m_margins.left(), m_margins.top(), -m_margins.right(),
                    -m_margins.bottom());
  ...
  checkbox_style.rect.setLeft(content_rect.x() + content_rect.width() -
                              checkbox_rect.width());
...
void PluginListDelegate::paint(QPainter *painter,
  ...
  painter->drawPixmap(content_rect.left(), content_rect.top(),
                      plugin_icon.pixmap(m_icon_size));
  ...
  QApplication::style()->drawControl(QStyle::CE_CheckBox, &checkbox_style,
                                     painter);
